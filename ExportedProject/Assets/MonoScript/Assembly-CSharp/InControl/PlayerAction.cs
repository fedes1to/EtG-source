...
namespace InControl
{
	public class PlayerAction : OneAxisInputControl
	{
		...
		public void RemoveBindingOfType(InputControlType control)
		{
			...
			{
				if (bindings[i] != null && bindings[i].BindingSourceType == BindingSourceType.DeviceBindingSource)
				{
					DeviceBindingSource deviceBindingSource = bindings[i] as DeviceBindingSource;
					if (deviceBindingSource.Control == control)
					{
						RemoveBinding(deviceBindingSource);
						break;
					}
				}
			}
			ForceUpdateVisibleBindings();
		}
		...
		public void ClearSpecificBindingByType(int index, params BindingSourceType[] types)
		{
			...
			{
				if (Array.IndexOf(types, regularBindings[i].BindingSourceType) >= 0)
				{
					if (num == index)
					{
		...
		public void SetBindingOfTypeByNumber(BindingSource binding, BindingSourceType sourceType, int index, Action<PlayerAction, BindingSource> OnBindingAdded = null)
		{
			...
			for (int i = 0; i < regularBindings.Count; i++)
			{
				if (list2.Contains(regularBindings[i].BindingSourceType))
				{
					list.Add(regularBindings[i]);
					regularBindings.RemoveAt(i);
					i--;
				}
			}
		...
		{
			m_ignoredBindingSources.Add(sourceType);
		}
		...
		public void ClearBindingsOfType(BindingSourceType sourceType)
		{
			for (int i = 0; i < regularBindings.Count; i++)
			{
				if (regularBindings[i].BindingSourceType == sourceType)
				{
					regularBindings.RemoveAt(i);
					i--;
				}
		...
		private void UpdateBindings(ulong updateTick, float deltaTime)
		{
			...
			for (int num2 = count - 1; num2 >= 0; num2--)
			{
				...
				bool flag2 = false;
				for (int i = 0; i < m_ignoredBindingSources.Count; i++)
				{
					if (m_ignoredBindingSources[i] == bindingSource.BindingSourceType)
					{
						flag2 = true;
					}
				}
			...
			{
				bool flag3 = bindingSourceType != LastInputType;
