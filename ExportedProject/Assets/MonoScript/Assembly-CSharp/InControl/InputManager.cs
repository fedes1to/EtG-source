...
namespace InControl
{
	public class InputManager
	{
		...
		private static void UpdateActiveDevice()
		{
			...
			if (ActiveDevice is XInputDevice && !GameManager.Options.allowXinputControllers)
			{
				ActiveDevice = null;
			}
			if (!(ActiveDevice is XInputDevice) && !GameManager.Options.allowNonXinputControllers)
			{
				ActiveDevice = null;
			}
			int count = devices.Count;
			for (int i = 0; i < count; i++)
			{
				...
				if ((!GameManager.Options.allowXinputControllers && inputDevice2 is XInputDevice) || (!GameManager.Options.allowNonXinputControllers && !(inputDevice2 is XInputDevice)) || !inputDevice2.LastChangedAfter(ActiveDevice) || inputDevice2.Passive)
				{
					continue;
				}
				if ((ActiveDevice is XInputDevice && !(inputDevice2 is XInputDevice)) || (inputDevice2 is XInputDevice && !(ActiveDevice is XInputDevice)))
				{
					if (inputDevice2.LastChangeAfterTime(ActiveDevice))
					{
			...
			if (GameManager.HasInstance && GameManager.Instance.CurrentGameType == GameManager.GameType.SINGLE_PLAYER)
			{
				...
				Debug.LogWarningFormat("swapping active device from {0} ({1}|{2}) to {3} ({4}|{5})", inputDevice.Name, inputDevice.LastChangeTick, inputDevice.LastChangeTime, ActiveDevice.Name, ActiveDevice.LastChangeTick, ActiveDevice.LastChangeTime);
			}
