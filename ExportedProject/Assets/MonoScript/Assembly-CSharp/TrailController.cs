...
public class TrailController : BraveBehaviour
{
	...
	public void Start()
	{
		...
		m_bones.AddLast(new Bone(base.specRigidbody.Position.UnitPosition + boneSpawnOffset, 0f, heightOffset));
		m_bones.AddLast(new Bone(base.specRigidbody.Position.UnitPosition + boneSpawnOffset, 0f, heightOffset));
		...
		speculativeRigidbody.OnCollision = (Action<CollisionData>)Delegate.Combine(speculativeRigidbody.OnCollision, new Action<CollisionData>(UpdateOnCollision));
		SpeculativeRigidbody speculativeRigidbody2 = base.specRigidbody;
		speculativeRigidbody2.OnPostRigidbodyMovement = (Action<SpeculativeRigidbody, Vector2, IntVector2>)Delegate.Combine(speculativeRigidbody2.OnPostRigidbodyMovement, new Action<SpeculativeRigidbody, Vector2, IntVector2>(PostRigidbodyMovement));
	}

	public void Update()
	{
		int num = Mathf.RoundToInt(m_sprite.GetCurrentSpriteDef().untrimmedBoundsDataExtents.x / m_sprite.GetCurrentSpriteDef().texelSize.x);
		int num2 = num / 4;
		...
		if (usesAnimation)
		{
			...
			while (linkedListNode != null)
			{
				...
				if (linkedListNode != null)
				{
					if (!linkedListNode.Value.IsAnimating && usesGlobalTimer && m_globalTimer > globalTimer)
					{
					...
					if (!linkedListNode.Value.IsAnimating && usesCascadeTimer && (linkedListNode == m_bones.First || num3 >= cascadeTimer))
					{
						linkedListNode.Value.IsAnimating = true;
						num3 = 0f;
						DoDispersalParticles(linkedListNode, num2);
					...
					if (!linkedListNode.Value.IsAnimating && usesSoftMaxLength && m_maxPosX - linkedListNode.Value.posX > softMaxLength)
					{
						linkedListNode.Value.IsAnimating = true;
						num3 = 0f;
						DoDispersalParticles(linkedListNode, num2);
	...
	private void HandleExtension(Vector2 specRigidbodyPosition)
	{
		...
		{
			float heightOffset = ((!rampHeight) ? 0f : rampStartHeight);
			m_bones.AddLast(new Bone(base.specRigidbody.Position.UnitPosition + boneSpawnOffset, m_maxPosX, heightOffset));
			m_bones.AddLast(new Bone(base.specRigidbody.Position.UnitPosition + boneSpawnOffset, m_maxPosX, heightOffset));
		}
		Vector2 vector = specRigidbodyPosition;
		if ((bool)base.specRigidbody.projectile && base.specRigidbody.projectile.OverrideTrailPoint.HasValue)
		{
			vector = base.specRigidbody.projectile.OverrideTrailPoint.Value;
		}
		ExtendBonesTo(vector + boneSpawnOffset);
		m_isDirty = true;
	}
	...
	public void SetTiledSpriteGeom(Vector3[] pos, Vector2[] uv, int offset, out Vector3 boundsCenter, out Vector3 boundsExtents, tk2dSpriteDefinition spriteDef, Vector3 scale, Vector2 dimensions, tk2dBaseSprite.Anchor anchor, float colliderOffsetZ, float colliderExtentZ)
	{
		...
		for (int i = 0; i < num4; i++)
		{
			...
			if (usesStartAnimation && i == 0)
			{
				...
				tk2dSpriteDefinition2 = m_sprite.Collection.spriteDefinitions[m_startAnimationClip.frames[num8].spriteId];
			}
			else if (usesAnimation && linkedListNode.Value.IsAnimating)
			{
				...
				tk2dSpriteDefinition2 = m_sprite.Collection.spriteDefinitions[m_animationClip.frames[num9].spriteId];
			}
			float num10 = 0f;
			for (int j = num6; j <= num7; j++)
			{
				...
				pos[num12++] = linkedListNode.Value.pos + linkedListNode.Value.normal * (tk2dSpriteDefinition2.position0.y * m_projectileScale) - m_minBonePosition;
				pos[num12++] = linkedListNode.Next.Value.pos + linkedListNode.Next.Value.normal * (tk2dSpriteDefinition2.position1.y * m_projectileScale) - m_minBonePosition;
				pos[num12++] = linkedListNode.Value.pos + linkedListNode.Value.normal * (tk2dSpriteDefinition2.position2.y * m_projectileScale) - m_minBonePosition;
				pos[num12++] = linkedListNode.Next.Value.pos + linkedListNode.Next.Value.normal * (tk2dSpriteDefinition2.position3.y * m_projectileScale) - m_minBonePosition;
				num12 = offset + num5;
