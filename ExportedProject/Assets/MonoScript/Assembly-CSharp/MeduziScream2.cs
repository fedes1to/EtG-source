	...
	private class TimedBullet : Bullet
	{
		...
		protected override IEnumerator Top()
		{
			if (m_bulletsFromSafeDir == 4)
			{
				if (m_hasArc)
				{
					BulletArcLightningController orAddComponent = Projectile.gameObject.GetOrAddComponent<BulletArcLightningController>();
					orAddComponent.Initialize(m_arcCenter, Speed, Projectile.OwnerName, m_startAngle, m_endAngle, 0.25f);
				}
	...
	protected override IEnumerator Top()
	{
		...
		for (int i = 0; i < 16; i++)
		{
			...
			for (int k = 0; k < 64; k++)
			{
				...
				if (num4 <= 4)
				{
					TimedBullet timedBullet = new TimedBullet(num4, Mathf.Sign(num3));
					if (num4 == 4 && Mathf.Sign(num3) >= 0f)
					{
						int num5 = Array.IndexOf(s_gapAngles, num2);
						int num6 = (num5 + 1) % s_gapAngles.Length;
						float startAngle = BraveMathCollege.ClampAngle360(s_gapAngles[num5] + (float)num4 * delta);
						float endAngle = BraveMathCollege.ClampAngle360(s_gapAngles[num6] - 3f * delta);
						timedBullet.SetArc(base.Position, startAngle, endAngle);
					}
					Fire(new Direction(num), new Speed(7f), timedBullet);
				}
			}
