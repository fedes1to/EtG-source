		...
		public void Init(int hash, int width, int height, int divX, int divY)
		{
			...
			for (int i = 0; i < numRows; i++)
			{
				for (int j = 0; j < numColumns; j++)
				{
					...
					int sY = i * divY;
					int eY = (i + 1) * divY;
					spriteChannel.chunks[i * numColumns + j] = new SpriteChunk(sX, sY, eX, eY);
				}
			}
		}
		...
		public void CreateOverrideChunk(SpriteChunk chunk)
		{
			...
			for (int i = 0; i < chunk.Width; i++)
			{
				for (int j = 0; j < chunk.Height; j++)
				{
					...
					int offset = 0;
					SpriteChunk spriteChunk = FindChunkAndCoordinate(intVector.x, intVector.y, out offset);
					if (offset >= 0 && offset < spriteChunk.spriteIds.Length)
					{
						chunk.spriteIds[j * chunk.Width + i] = spriteChunk.spriteIds[offset];
						num++;
					}
					else
					{
