...
public class tk2dButton : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Input.multiTouchEnabled)
		{
			for (int i = 0; i < Input.touchCount; i++)
			{
				...
				if (touch.phase == TouchPhase.Began)
				{
					...
					RaycastHit hitInfo;
					if (GetComponent<Collider>().Raycast(ray, out hitInfo, 1E+08f) && !Physics.Raycast(ray, hitInfo.distance - 0.01f))
					{
		...
		if (!flag && Input.GetMouseButtonDown(0))
		{
			...
			RaycastHit hitInfo2;
			if (GetComponent<Collider>().Raycast(ray2, out hitInfo2, 1E+08f) && !Physics.Raycast(ray2, hitInfo2.distance - 0.01f))
			{
